...
class GenreStatsResponse(BaseModel):
    ...
    total_gross_sum: int = Field(..., description="Total Gross (Money) for Genre.")
    avg_gross: float = Field(..., description="Total Average (Promedio) for Movie")
...
class StatsStaticsResponse(BaseModel):
    ...
    top_grossing: MovieResponse = Field(
        ..., description="Movies with many gros (Mayor recaudación)"
    )
    most_recent: MovieResponse = Field(
        ..., description="Movies most recent in the Dataset (DB)"
    )
...
class ErrorResponse(BaseModel):
    ...
    model_config = {
        "json_schema_extra": {
            "example": {
                "detail": "CSV not found at path /app/data/disney_movies_2_cleaned.csv",
                "code": 404,
            }
        }
    }
