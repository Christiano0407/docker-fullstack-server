...
def list_movies(
    ...
    offset: int = Query(
        default=0, ge=0, description="Number to Register to skip(Omitir)"
    ),
):
    try:
        rows, total = get_data_paginated(limit=limit, offset=offset)
        return MovieListResponse(
            count=len(rows), total=total, limit=limit, offset=offset, data=rows
        )
...
def search(
    genre: str | None = Query(
        default=None, description="Filter by Genre (ex:Adventure, Comedy)"
    ),
    rating: str | None = Query(
        default=None, description="Filter by rating MPAA(G, PG, PG-13, R)"
    ),
    limit: int = Query(default=10, ge=1, le=100),
    offset: int = Query(default=0, ge=0),
):
    try:
        rows, total = search_movies(
            genre=genre, rating=rating, limit=limit, offset=offset
        )
        if not rows:
            raise HTTPException(
                status_code=404, detail="Not movies found with the given filters"
            )
        return MovieListResponse(
            count=len(rows), total=total, limit=limit, offset=offset, data=rows
        )
